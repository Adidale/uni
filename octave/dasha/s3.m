...
for i = 1:length(P1)
  ...
  EQFL1 = FQ + FRA + FRC1 == [0 0 0];
  EQML1 = cross(G,FQ) + cross (A, FRA) + cross (C, FRC1) + MM == [0 0 0];
  EQFR1 = FRB - FRC1 + FP1 == [0 0 0];
  EQMR1 = cross(D,FP1) + cross(B,FRB) - cross(C,FRC1) == [0 0 0];
  ...
  EQFL2 = FQ + FRA + FRC2 == [0 0 0];
  EQML2 = cross(G,FQ) + cross (A, FRA) + cross (C, FRC2) + MM + MMCR2 == [0 0 0];
  EQFR2 = FRB + FRC2 + FP1 == [0 0 0];
  EQMR2 = cross(D,FP1) + cross(B,FRB) - cross(C,FRC2) - MMCR2 == [0 0 0];

  Sys2= [EQFL2(1) EQFL2(2) EQFR2(1) EQFR2(2) EQML2(3) EQMR2(3)];
  solution2 = vpasolve(Sys2, [XA YA XB YB XC MC]);
